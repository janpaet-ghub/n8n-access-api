 ...
 */
function logDbError(err, { sql, action }) {
  const meta = connection.__meta || {};
  // Print a compact but useful error context
  console.error("[DB] ERROR:", {
    action,
    message: String(err?.message || err),
    mode: meta.mode,
    provider: meta.provider,
    dbPath: meta.dbPath, // only present in ACCESS_DB_PATH mode
  });
 ...
 */
async function updateOrderComment({ kdNr, auftragsID, comment }) {
...
`.trim();
  ...
  try {
    ...
    logDbError(err, { sql, action: "updateOrderComment" });
    throw err;
  }
}
